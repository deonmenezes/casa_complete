...
const ProductsPage: React.FC = () => {
  ...
  const fetchProductsByCategory = async (categoryId: string) => {
    try {
      const response = await fetch(`http://https://casa-backend-uf0h.onrender.com/api/products/category?category=${categoryId}`);
  ...
  const fetchProductsByBrand = async (brandId: string) => {
    try {
      const response = await fetch(`http://https://casa-backend-uf0h.onrender.com/api/products/brand?id=${brandId}`);
      const data = await response.json();
      setProducts(data);
    } catch (error) {
      console.error('Error fetching products:', error);
