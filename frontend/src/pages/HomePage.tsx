...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      ...
      try {
        const allRes = await fetch("https://casa-backend-uf0h.onrender.com/api/brands");
        ...
        const latestRes = await fetch("https://casa-backend-uf0h.onrender.com/api/brands?sort=latest");
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      try {
        const response = await axios.get('https://casa-backend-uf0h.onrender.com/api/categories');
  ...
  const handleCategoryClick = async (categoryId: string) => {
    try {
      const res = await fetch(`https://casa-backend-uf0h.onrender.com/api/products/category?category=${categoryId}`);
